 ...
 */
package com.js.worker.code.diruptor.api;
...
public class DisruptorQueueImpl extends DisruptorQueue {
	...
	protected final RingBuffer<MutableObject> _buffer;
	protected final Sequence _consumer;
	protected final SequenceBarrier _barrier;
	...
	public DisruptorQueueImpl(String queueName, ProducerType producerType, int bufferSize, WaitStrategy wait) {
		...
		/*
		 * if (producerType == ProducerType.SINGLE) { consumerStartedFlag =
		 * true; } else { // make sure we flush the pending messages in cache
		 * first if (bufferSize < 2) { throw new RuntimeException(
		 * "QueueSize must >= 2"); } try { publishDirect(FLUSH_CACHE, true); }
		 * catch (InsufficientCapacityException e) { throw new RuntimeException(
		 * "This code should be unreachable!", e); } }
		 */
	...
	public void consumeBatchToCursor(long cursor, EventHandler<Object> handler) {
		for (long curr = _consumer.get() + 1; curr <= cursor; curr++) {
			try {
				...
				mo.setObject(null);
				/*
				 * if (o == FLUSH_CACHE) { Object c = null; while (true) { c =
				 * _cache.poll(); if (c == null) break; else handler.onEvent(c,
				 * curr, true); } } else if (o == INTERRUPT) { throw new
				 * InterruptedException("Disruptor processing interrupted"); }
				 * else {
				 */
	...
	public void publish(Object obj, byte[] ser) {
		publish(obj);
	}
	...
	public void publish(Object obj, boolean block) throws InsufficientCapacityException {
		 ...
		 * 
		 * if (!publishNow) { readLock.lock(); try { publishNow =
		 * consumerStartedFlag; if (!publishNow) { _cache.add(obj); } } finally
		 * { readLock.unlock(); } }
		 * 
		 * if (publishNow) {
		 */
	 ...
	 * 
	 * writeLock.lock(); consumerStartedFlag = true;
	 * 
	 * writeLock.unlock(); }
	 */
